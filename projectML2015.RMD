...
df_training<-cbind(training3[,-c(columnsnumeric)],train.pca,classe) ##variables correlated are deleted
...
rf.cv <- rfcv(df_training[,-c(classe)], df_training$classe, cv.fold=10)
with(rf.cv, plot(n.var, error.cv, type="b", col="red"))

```

The Cross validation is plotted and found that the mean of error.cv is
**`r mean(rf.cv$error.cv)`** with any number of variables. This is an excelent result and supports to Random Forest as chosen model.
...
confussion.training<-confusionMatrix(rf.pred.training, df_training$classe)
confussion.training
...
df_testing<-cbind(testing_sample2[,-c(columnsnumeric)],testing.pca,classe)

set.seed(12345)
rf.pred=predict(modfit,df_testing,type="class")
predMatrix = with(df_testing,table(rf.pred,classe))
confussion.testing<-confusionMatrix(rf.pred, df_testing$classe)
confussion.testing
```
